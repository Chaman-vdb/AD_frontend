...
function ReplicationForm({ orgDetails, companyDetails, isReplicating, onReplicate, plans = {} }) {
    ...
    const validateOrgName = async () => {
        ...
        try {
            const res = await fetch(apiUrl(`/api/data/validate/org-name?name=${encodeURIComponent(newOrgName)}`));
    ...
    const validateCompanyName = async () => {
        ...
        try {
            const res = await fetch(apiUrl(`/api/data/validate/company-name?name=${encodeURIComponent(newCompanyName)}`));
            const data = await res.json();
            if (data.exists) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const orgRes = await fetch(apiUrl(`/api/data/validate/org-name?name=${encodeURIComponent(newOrgName)}`));
            ...
            if (companyDetails && newCompanyName) {
                const compRes = await fetch(apiUrl(`/api/data/validate/company-name?name=${encodeURIComponent(newCompanyName)}`));
