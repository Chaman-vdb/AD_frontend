...
function CopyCompanyForm({ orgs, isReplicating, onCopyCompany, onDetailsChange }) {
    ...
    useEffect(() => {
        ...
        fetch(apiUrl(`/api/data/orgs/${sourceOrgId}`)).then(r => r.json()).then(setSourceOrgDetails).catch(console.error);
        fetch(apiUrl(`/api/data/companies?org_id=${sourceOrgId}`)).then(r => r.json()).then(setCompanies).catch(console.error).finally(() => setLoadingCompanies(false));
    }, [sourceOrgId]);

    useEffect(() => {
        if (!selectedCompany) { setCompanyDetails(null); setNewCompanyName(''); return; }
        fetch(apiUrl(`/api/data/companies/${selectedCompany}`)).then(r => r.json()).then(d => { setCompanyDetails(d); setNewCompanyName(`Copy of ${d.name}`); }).catch(console.error);
    }, [selectedCompany]);

    useEffect(() => {
        if (!destOrgId) { setDestOrgDetails(null); return; }
        fetch(apiUrl(`/api/data/orgs/${destOrgId}`)).then(r => r.json()).then(setDestOrgDetails).catch(console.error);
    }, [destOrgId]);
    ...
    const validateCompanyName = async () => {
        ...
        try {
            const res = await fetch(apiUrl(`/api/data/validate/company-name?name=${encodeURIComponent(newCompanyName)}&org_id=${destOrgId}`));
